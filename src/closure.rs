...
pub fn take_fourth() {
    ...
    let my_vec = my_vec.iter()
        .map(|x| x * 2)
        .collect::<Vec<i32>>();
    my_vec.iter()
        .enumerate()
        .for_each(|(index, number)| {
            println!("Index number {index} has number {number}")
        });
...
pub fn filter_map_demo1() {
    ...
    company_vec.iter().for_each(|company| result_vec.push(company.get_ceo().ok_or("No Ceo Found!")));
    company_vec.iter().for_each(
        |company| result_vec1.push(company.get_ceo().ok_or_else(|| {
...
pub fn filter_map_demo2() {
    ...
    let actual_number = user_input.into_iter()
...
pub fn closure_demo4() {
    ...
    println!("All alphabetic? {}", smaller_vec.iter().all(|e| e.is_alphabetic()));
    println!("All less than the character 国? {}", smaller_vec.iter().all(|e| e < &'国'));
}
...
pub fn cycle_vec() {
    ...
    let even_odd_vec = (0..6).zip(even_odd.into_iter().cycle())
        .collect::<Vec<(i32, &str)>>();
...
pub fn peek_vector_demo() {
    ...
    iter.next();
    if let Some(pointer) = iter.peek_mut() { // get a mut reference to next element
        *pointer = &1000;
    }
    println!("The second number was updated, now it is {}", iter.peek().unwrap());

    let locations = [("Nevis", 25),
        ("Taber", 8428),
        ("Markerville", 45),
        ("Cardston", 3585)];
...
pub fn closure_function_demo(){
    ...
    tallinn.city_data(|city_years, city_populations| {
        let new_vec : Vec<(_,_)> = city_years.iter().zip(city_populations.iter()).take(5).collect();
        println!("{:?}", new_vec);
